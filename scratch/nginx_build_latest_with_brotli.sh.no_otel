...
# Removed -funroll-loops -ffunction-sections -fdata-sections -Wl,--gc-sections -Ofast -fPIE

# Set Flags
export CFLAGS="-march=native -mtune=native -fPIC"
...
./configure \
  --prefix=/etc/nginx \
  --with-cc-opt="-O3 -fstack-protector-strong -Wformat -Werror=format-security" \
...
echo "Build - doing post_setup.";

sudo mkdir /etc/nginx/sites-available
sudo mkdir /etc/nginx/sites-enabled

# Copy default configs
sudo cp /arrow-next/nginx/nginx.conf /etc/nginx/nginx.conf
sudo cp /arrow-next/nginx/default.conf /etc/nginx/sites-available/default.conf

# Create symlinks
sudo ln -s /etc/nginx/sites-available/default.conf /etc/nginx/sites-enabled/default.conf
sudo ln -s /arrow-next/nginx/arrowelectrical.co.uk.conf /etc/nginx/sites-enabled/arrowelectrical.co.uk.conf


sudo nginx -t

echo "Start with sudo service restart nginx"